...
class TestObservation:
    def test_never_looked_is_distinguishable_from_looked_and_empty(self):
        assert ledger.known_ids(USER, DOC) is None
        ledger.observe(USER, DOC, [])
        assert ledger.known_ids(USER, DOC) == frozenset()

    def test_observe_replaces_the_whole_set(self):
        ledger.observe(USER, DOC, [RECORD_A, RECORD_B])
        assert ledger.known_ids(USER, DOC) == {"sug.a", "sug.b"}
        ledger.observe(USER, DOC, [RECORD_B])
        assert ledger.known_ids(USER, DOC) == {"sug.b"}

    def test_only_the_echo_fields_are_cached(self):
        ledger.observe(USER, DOC, [RECORD_A])
    ...
    def test_records_without_an_id_are_ignored(self):
        ledger.observe(USER, DOC, [{"pre_text": "orphan"}, None, RECORD_A])
        assert ledger.known_ids(USER, DOC) == {"sug.a"}
...
class TestIsolationAndBounds:
    ...
    def test_documents_are_tracked_separately(self):
        ledger.observe(USER, DOC, [RECORD_A])
        assert ledger.known_ids(USER, "doc-2") is None

    def test_document_count_is_bounded(self):
        for n in range(ledger.MAX_DOCUMENTS + 10):
            ledger.observe(USER, f"doc-{n}", [RECORD_A])
        assert len(ledger._entries) == ledger.MAX_DOCUMENTS
        # The oldest went first; the newest is still there.
        assert ledger.known_ids(USER, "doc-0") is None
        assert ledger.known_ids(USER, f"doc-{ledger.MAX_DOCUMENTS + 9}") == {"sug.a"}

    def test_touching_a_document_keeps_it_alive(self):
        ledger.observe(USER, "doc-keep", [RECORD_A])
        for n in range(ledger.MAX_DOCUMENTS - 1):
            ledger.observe(USER, f"doc-{n}", [RECORD_A])
        ledger.observe(USER, "doc-keep", [RECORD_A])  # re-touch
        ledger.observe(USER, "doc-overflow", [RECORD_A])
        assert ledger.known_ids(USER, "doc-keep") == {"sug.a"}
